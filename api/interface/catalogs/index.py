...
def get(environment:dict, path_params:dict, query_params:dict, body) -> Response:
  ...
  catalog_name                    = param_val_errors[GetCatalogsQueryParams.CATALOG_NAME.param_name]
  catalog_name_has_wildcards      = param_val_errors[GetCatalogsQueryParams.CATALOG_NAME_HAS_WILDCARDS.param_name]
  catalog_name_is_case_sensitive  = param_val_errors[GetCatalogsQueryParams.CATALOG_NAME_IS_CASE_SENSITIVE.param_name]
  catalog_name_matches_diacritics = param_val_errors[GetCatalogsQueryParams.CATALOG_NAME_MATCHES_DIACRITICS.param_name]
  ...
  order_by    = param_val_errors[GetCatalogsQueryParams.ORDER_BY.param_name][0]
  ...
  return Response(Catalogs(catalogs), HTTPStatusCodes.HTTP200)
