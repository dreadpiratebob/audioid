...
function queue_stuff_up($data, $sql_link)
{
  ...
  $query    = "SELECT s.id AS id, s.name AS title, s.filename AS filename, GROUP_CONCAT(ar.name, s_ar.conjunction ORDER BY s_ar.list_order SEPARATOR '') AS artist\n" .
              ...
              "WHERE s.id=$id\n" .
              "GROUP BY s.id, s.name, s.filename"; // gotta have 3 columns here 'cause i can't select columns that i'm not also grouping by.  also, i have to have the 'group by' clause after the 'where' clause.
  ...
  $song = $sql_song->fetch(PDO::FETCH_ASSOC);
